...
GetFixedPar_Uni<-function(Dist,GASPar){
  ...
  if(Dist == "gamma"){
    if(!GASPar$scale) FixedPar = c(FixedPar, a1=0, b1=0)
    if(!GASPar$shape) FixedPar = c(FixedPar, a2=0, b2=0)
  }
  if(Dist == "exp"){
    if(!GASPar$location) FixedPar = c(FixedPar, a1=0,b1=0)
  }
...
MultiFixedScale<-function(iN,Dist){
  ...
  if(ScalarParameters){
    ...
    FixedPar = c(0, 0);
    if(Dist == "mvnorm")  names(FixedPar) = c("a.sigma", "b.sigma")
    if(Dist == "mvt")     names(FixedPar) = c("a.phi", "b.phi")
...
GetFixedPar_Multi<-function(Dist,GASPar,iN, ScalarParameters){
  ...
  if(!GASPar$location)    FixedPar = c(FixedPar, MultiFixedLocation(iN, ScalarParameters))
  if(!GASPar$scale)       FixedPar = c(FixedPar, MultiFixedScale(iN,Dist, ScalarParameters))
  if(!GASPar$correlation) FixedPar = c(FixedPar, MultiFixedCorrelation(iN, ScalarParameters))
...
MatrixCoefficientStructure_Multi<-function(Dist, iN, iK, GASPar){
  ...
  if(GASPar$location)    diag(lStructure[["location"]])[1:iN] = TRUE
  if(GASPar$scale)       diag(lStructure[["scale"]])[(iN + 1):(2*iN)] = TRUE
  if(GASPar$correlation) diag(lStructure[["correlation"]])[(2*iN + 1):(2*iN + iN * (iN - 1) / 2)] = TRUE
  if(GASPar$shape)       diag(lStructure[["shape"]])[(1 + 2*iN + iN * (iN - 1) / 2)] = TRUE
