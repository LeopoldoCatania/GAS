StartingNu<-function(vY){
  dNu = try(exp(solnp(log(12.0),
                      function(x,vY){
                        dNu = exp(x) + LowerNu()
                        -sum( dt( vY, dNu, log=TRUE ) )
                      },
                      vY = vY, control = list(trace = 0))$pars)
            + LowerNu(),
            silent = TRUE)
...
UniGAS_Starting<-function(vY,iT,iK,Dist,ScalingType, GASPar){
  ...
  if(iK>1){
    vA     = starting_vA_Uni(vY, vKappa, mB=diag(rep(9e-1,iK)), dA_foo=1e-6, iT, iK, Dist, ScalingType=ScalingType, GASPar)
    vB     = starting_vB_Uni(vY, vKappa, dB_foo=0.9, mA=diag(vA), iT, iK, Dist,ScalingType=ScalingType, GASPar)
  }else{
    vA     = starting_vA_Uni(vY, vKappa, mB=matrix(9e-1,iK,iK), dA_foo=1e-6, iT, iK, Dist, ScalingType=ScalingType, GASPar)
...
starting_vA_Uni<-function(vY, vKappa, mB, dA_foo, iT, iK, Dist,ScalingType, GASPar){
  ...
  seq_alpha = c(seq(1e-4,1.5,length.out = 30))
      ...
      for(l in 1:length(seq_alpha)){
        ...
        if( is.numeric(dLLK_foo) & !is.nan(dLLK_foo) ){
          ...
          if(is.numeric(dLLK_post) & !is.nan(dLLK_post)){
            ...
            if(dLLK_post>dLLK_foo) dAlpha_best = seq_alpha[l]
      ...
      for(l in 1:length(seq_beta)){
        ...
        if(is.numeric(dLLK_foo) & !is.nan(dLLK_foo)){
          ...
          if(is.numeric(dLLK_post) & !is.nan(dLLK_post)){
            ...
            if(dLLK_post>dLLK_foo)  dB_best = seq_beta[l]
...
StartingValues_mvnorm<-function(mY,iT, iN, iK,GASPar, ScalingType, ScalarParameters){
  ...
  if(ScalarParameters){
    ...
    mA = starting_mA_Multi_Scalars(mY, vKappa, mB = diag(rep(0.9,iK)), dA_foo = 1e-3, iT, iK, iN, "mvnorm",ScalingType, GASPar)
    ...
    vA = starting_vA_Multi(mY, vKappa, mB = diag(rep(0.9,iK)), dA_foo = 1e-2, iT, iK, iN, "mvnorm", ScalingType, GASPar)
    vB = starting_vB_Multi(mY, vKappa, dB_foo = 0.90, mA = diag(vA), iT, iK, iN, "mvnorm", ScalingType, GASPar)
...
StartingValues_mvt<-function(mY,iT, iN, iK,GASPar, ScalingType, ScalarParameters){
  ...
  if(ScalarParameters){
    ...
    mA = starting_mA_Multi_Scalars(mY, vKappa, mB = diag(rep(0.9,iK)), dA_foo = 1e-3, iT, iK, iN, "mvt",ScalingType, GASPar)
    ...
    vA = starting_vA_Multi(mY, vKappa, mB = diag(rep(0.9,iK)), dA_foo = 1e-2, iT, iK, iN, "mvt", ScalingType, GASPar)
    vB = starting_vB_Multi(mY, vKappa, dB_foo = 0.90, mA = diag(vA), iT, iK, iN, "mvt", ScalingType, GASPar)

  }

  vA = unmapVec_C(vA, LowerA(), UpperA()); names(vA) = paste("a.",mvtParNames(iN, ScalarParameters),sep="")
  vB = unmapVec_C(vB, LowerB(), UpperB()); names(vB) = paste("b.",mvtParNames(iN, ScalarParameters),sep="")
...
MultiGAS_Starting<-function(mY,iT, iN, iK,Dist, GASPar, ScalingType, ScalarParameters){
  ...
  if(Dist=="mvnorm") vPw = StartingValues_mvnorm(mY,iT, iN, iK,  GASPar, ScalingType, ScalarParameters)
  if(Dist=="mvt")    vPw = StartingValues_mvt(mY,iT, iN, iK,  GASPar, ScalingType, ScalarParameters)
      ...
      for(l in 1:length(seq_beta)){
        ...
        if(is.numeric(dLLK_foo) & !is.nan(dLLK_foo)){
          ...
          if(is.numeric(dLLK_post) & !is.nan(dLLK_post)){
            ...
            if(dLLK_post>dLLK_foo) dBeta_best = seq_beta[l]
