...
MultiGASFor<-function(mGASFit, iH, Roll = F, mOut = NULL,iB = 10000,
                      vBands = c(0.1,0.15,0.85,0.9), bReturnsDraws = F){
  ...
  if(Roll) {
    ...
    if(bReturnsDraws){cY = Forc$cY}else{cY = array(0,dim = c(1,1,1))}
  ...
  Out <- new("mGASFor",
             Forecast = list(PointForecast = PointForecast, Moments = lMoments, vLS=vLS),
             Bands    = cBands,
             Draws    = cY,
             Info     = list(iH = iH, iN = iN,Roll = Roll, iB = iB, vBands = vBands,
                             bReturnsDraws = bReturnsDraws, GASPar=GASPar,
                             Dist = Dist, ScalingType = ScalingType, iK = iK),
...
UniGASRoll<-function(vY,GASSpec,ForecastLength = 500, Nstart = NULL, RefitEvery = 23, RefitWindow = c("recursive", "moving"),
  ...
  Out <- new("uGASRoll",
             Forecast = list(PointForecast = mForc, vU = vU, vLS = vLS, Moments=Moments, Coef = lCoef),
...
MultiGASRoll<-function(mY,GASSpec,ForecastLength = 500, Nstart = NULL, RefitEvery = 23, RefitWindow = c("recursive", "moving"),
                       cluster=NULL){
  ...
  #
  #fits
  if(is.null(cluster))  lFits=lapply(lData,MultiGASFit, GASSpec=GASSpec)
  if(!is.null(cluster)) lFits=parLapply(cluster,lData,MultiGASFit,GASSpec=GASSpec)
  ...
  Out <- new("mGASRoll",
             ...
             Info = list(GASSpec = GASSpec, ForecastLength = ForecastLength,
                         RefitEvery = RefitEvery, RefitWindow = RefitWindow[1],
                         iT = iT, iK = iK, iN = iN, elapsedTime = elapsedTime),
             Data = list(mY = mY))
  return(Out)
