UniGASFor <- function(uGASFit, H, Roll = F, out = NULL, B = 1000, Bands = c(0.1, 0.15, 0.85, 0.9), ReturnDraws = FALSE) {
    ...
    Out <- new("uGASFor", Forecast = list(PointForecast = PointForecast, Moments = mMoments, vU = vU,
        vLS = vLS), Bands = cBands, Draws = mY, Info = list(iH = iH, Roll = Roll, iB = iB, vBands = vBands,
        bReturnDraws = bReturnDraws, GASPar = GASPar, Dist = Dist, ScalingType = ScalingType, iK = iK),
...
MultiGASFor <- function(mGASFit, H, Roll = FALSE, out = NULL, B = 10000, Bands = c(0.1, 0.15, 0.85, 0.9),
    ReturnDraws = FALSE) {
    ...
    if( any(class(out)[1] == c("xts", "ts", "zoo"))) PointForecast = xts(PointForecast, order.by = index(out))

    lMoments = EvalMoments_multi(t(PointForecast), Dist, iN)

    Out <- new("mGASFor", Forecast = list(PointForecast = PointForecast, Moments = lMoments, vLS = vLS),
        Bands = cBands, Draws = cY, Info = list(iH = iH, iN = iN, Roll = Roll, iB = iB, vBands = vBands,
            bReturnDraws = bReturnDraws, GASPar = GASPar, Dist = Dist, ScalingType = ScalingType, iK = iK),
        Data = list(mY = mY, FilteredParameters = FilteredParameters, mOut = mOut))
...
UniGASRoll <- function(data, GASSpec, ForecastLength = 500, Nstart = NULL, RefitEvery = 23, RefitWindow = c("moving",
    ...
    Out <- new("uGASRoll", Forecast = list(PointForecast = mForc, vU = vU, vLS = vLS, Moments = Moments,
        Coef = lCoef), Info = list(GASSpec = GASSpec, ForecastLength = ForecastLength, RefitEvery = RefitEvery,
        RefitWindow = RefitWindow[1], iT = iT, iK = iK, elapsedTime = elapsedTime), Testing = list(PitTest = PitTest),
        Data = list(vY = vY))
    return(Out)
...
MultiGASRoll <- function(data, GASSpec, ForecastLength = 500, Nstart = NULL, RefitEvery = 23, RefitWindow = c("moving",
    ...
    Out <- new("mGASRoll", Forecast = list(PointForecast = mForc, vU = vU, vLS = vLS, Moments = Moments,
        Coef = lCoef), Info = list(GASSpec = GASSpec, ForecastLength = ForecastLength, RefitEvery = RefitEvery,
        RefitWindow = RefitWindow[1], iT = iT, iK = iK, iN = iN, elapsedTime = elapsedTime), Data = list(mY = mY))
    return(Out)
