context("Test Information Matrix")
...
test_that("Information Matrix", {
  ...
  #
  # mApprox = round(matrix(cubature::adaptIntegrate(function(dY, vTheta) {
  #   vScore = GAS::Score_Uni(dY, vTheta, "snorm")
  #   vScore %*% t(vScore) * GAS::ddist_Uni(dY, vTheta, "snorm", log = FALSE)
  # }, lowerLimit = -15, upperLimit = 15, vTheta = vTheta, fDim = length(vTheta)^2)$integral,
  # length(vTheta)), 4)
  #
  # mGAS = GAS:::IM_Uni(vTheta, "snorm")
  #
  # expect_equal(dim(mApprox), c(3, 3))
  # expect_equal(dim(mGAS), c(3, 3))
  # expect_true(max(mApprox - mGAS) < tol)
